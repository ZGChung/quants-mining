    ...
    def fetch(self, ticker: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        ...
        try:
            ...
            if "Time Series (Daily)" in data:
                ...
                logger.warning(
                    f"Alpha Vantage no data for {ticker}: {data.get('Note', data.get('Error', 'Unknown'))}"
                )
...
class PolygonDataSource(DataSource):
    ...
    def fetch(self, ticker: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        ...
        # 转换周期
        multiplier, timespan = {"1d": (1, "day"), "1h": (1, "hour"), "5m": (5, "minute")}.get(
            interval, (1, "day")
        )
...
class MockDataSource(DataSource):
    ...
    def fetch(self, ticker: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        days = {"1mo": 30, "3mo": 90, "6mo": 180, "1y": 365, "2y": 730}
        start_date = (pd.Timestamp.now() - pd.Timedelta(days=days.get(period, 365))).strftime(
            "%Y-%m-%d"
        )
...
class DataSourceFactory:
    ...
    def create(cls, source: str = "mock", **kwargs) -> DataSource:
        ...
        if source not in cls._sources:
            raise ValueError(
                f"Unknown data source: {source}. Available: {list(cls._sources.keys())}"
            )
