...
class PaperTrader:
    ...
    def get_portfolio_value(self, prices: Dict[str, float]) -> float:
        """组合市值"""
        positions_value = sum(self.positions.get(t, 0) * prices.get(t, 0) for t in self.positions)
    ...
    def buy(self, ticker: str, quantity: int, price: float, signal_reason: str = "") -> bool:
        """买入"""
        cost = quantity * price * (1 + self.slippage + self.commission)
    ...
    def sell(self, ticker: str, quantity: int, price: float, signal_reason: str = "") -> bool:
        """卖出"""
        ...
        self.portfolio_history.append(
            {
                ...
                "cash": self.cash,
                "positions_value": sum(
                    self.positions.get(t, 0) * prices.get(t, 0) for t in self.positions
                ),
