...
class ResultExporter:
    ...
    def export_backtest_result(
        self, result: Dict, strategy_name: str, tickers: List[str], filename: Optional[str] = None
    ) -> str:
        """
        导出回测结果
    ...
    def export_optimization_result(
        self, optimization_result, strategy_name: str, filename: Optional[str] = None
    ) -> str:
        """
        导出优化结果
        """
    ...
    def export_trades(self, trades: List, filename: Optional[str] = None) -> str:
        """
        导出交易记录
        """
        if filename is None:
            ...
            trades_data.append(
                {
                    ...
                    "ticker": t.ticker,
                    "action": (
                        t.action if hasattr(t, "action") else ("BUY" if t.quantity > 0 else "SELL")
                    ),
    ...
    def create_report(self, results: Dict[str, Dict], filename: str = "strategy_comparison") -> str:
        """
        创建策略对比报告
        """
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
