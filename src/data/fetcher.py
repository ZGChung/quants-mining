...
class StockDataFetcher:
    ...
    def __init__(self, cache_dir: Optional[str] = None, retry_delay: float = 2.0, max_retries: int = 3):
        """
        初始化数据获取器
            ...
            try:
                ...
                if "rate limit" in error_msg.lower() or "too many requests" in error_msg.lower():
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(f"Rate limited for {ticker}, retrying in {wait_time}s... (attempt {attempt+1}/{self.max_retries})")
